...
public class HttpUtil {
	...
	public static String securePostByAuth(String url, Map<String, String> params) {
		...
		try {
			...
			File file = new File(HttpUtil.class.getClassLoader().getResource("security/server.keystore").toURI());
			String keyPwd = "123456";
			// Trust own CA and all self-signed certs
			SSLContext sslcontext = SSLContexts.custom().loadTrustMaterial(file, keyPwd.toCharArray(), new TrustSelfSignedStrategy()).build();
	...
	public static void downloadFile(String url, Map<String, String> params, File file) {
		...
		try {
			...
			httpclient = HttpClients.createDefault();
			HttpPost request = new HttpPost(url);
			RequestConfig config = RequestConfig.custom().setConnectTimeout(DEFAULT_CONNECTION_TIMEOUT).setSocketTimeout(60 * 1000 * 10).build();
			request.setConfig(config);
			request.setEntity(new UrlEncodedFormEntity(nvps, DEFAULT_ENCODE));
			response = httpclient.execute(request);
